...
'''会员集点活动数目'''
def get_redpoint_count(driver):
    try:
        common.findCss(driver,"body > div.top_content > div.main > div:nth-child(2) > div > div.panel > div.pagination_wrapper > div > span.el-pagination__sizes > div > div.el-input > i")
        a = True
    except:
        a = False
    if a == True:
        redpoint_page_btn = common.findCss(driver,"body > div.top_content > div.main > div:nth-child(2) > div > div.panel > div.pagination_wrapper > div > span.el-pagination__sizes > div > div.el-input > i")
        redpoint_page_btn.click()
        time.sleep(1)
        redpoint_page100_btn = common.findXpath(driver, "/html/body/div[4]/div/div[1]/ul/li[6]/span")
        redpoint_page100_btn.click()
        time.sleep(2)
        redpoint_count = common.findsCss(driver,"#memberredcollect > div.el-table__body-wrapper > table > tbody > tr")
        return len(redpoint_count)
    elif a == False:
        redpoint_count = common.findsCss(driver,"#memberredcollect > div.el-table__body-wrapper > table > tbody > tr")
        return len(redpoint_count)
    else:
...
'''会员集点店铺活动数目'''
def get_redpoint_shopname_count(driver):
        redpoint_shopname_count = common.findsCss(driver,"#memberredcollect > div.el-table__body-wrapper > table > tbody > tr")
        return len(redpoint_shopname_count)
...
def redpoint_shopname_filter(driver, redpoint_shopname):
    if redpoint_shopname == u"全部":
        ...
        time.sleep(1)
        driver.find_element_by_xpath("/html/body/div[4]/div/div[1]/ul/li[1]/span").click()
...
'''会员集点修改活动'''
def edit_redpoint(driver,dictory2):
    # 修改集点
    ...
    point_present_name.send_keys(dictory2["present_name"])

    point_present_amount = driver.find_element_by_xpath("/html/body/div[1]/div[2]/div[2]/div/div[2]/div[2]/div/form/div[5]/div/div[1]/input")
    point_present_amount.clear()
    point_present_amount.send_keys(dictory2["present_amount"])
