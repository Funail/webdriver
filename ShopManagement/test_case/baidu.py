...
class Baidu(unittest.TestCase):
    ...
    def testUser_Info(self):
        driver=self.driver
        driver.get('http://www.baidu.com')
        common.findID(driver, user.soID).send_keys('webdriver')
        common.findID(driver, user.clickID).click()
        time.sleep(3)

    def testInputName(self):
        driver=self.driver
        driver.get('https://sh.qfpay.com')
        driver.find_element_by_xpath('/html/body/div/div/form/div[3]').click()
        text=driver.find_element_by_xpath('/html/body/div/div/form/div[1]/div/div/input').text
        if text==function.inputName:
            print '测试通过'
        else:
            print '提示信息错误，请提单跟踪!'
...
if __name__=='__main__':
    ...
    #定义报告存放路劲
    filename="report/'+now+'Tests.html"
    fp=file(filename,'wb')
